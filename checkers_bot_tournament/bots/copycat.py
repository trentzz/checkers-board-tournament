...
class CopyCat(Bot):
    ...
    def get_mirror_move(self, board_size: int, move: Move) -> Move:
        mirrored_start = (board_size - 1 - move.start[0], board_size - 1 - move.start[1])
        ...
        if move.removed:
            mirrored_removed = (board_size - 1 - move.removed[0], board_size - 1 - move.removed[1])
    ...
    def play_move(self, board: Board, colour: str, move_list: list[Move]) -> int:
        ...
        mirror_move = self.get_mirror_move(board.size, prev_move)
