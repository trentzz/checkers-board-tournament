...
from checkers_bot_tournament.bots.bot_tracker import BotTracker, EloConfig
...
def compute_performance_rating(w: int, d: int, l: int, bot_rating: float, opp_rating: float):
    ...
    R_i(R_1, R_2, ..., R_n), the perfect performance rating
    is the number R_A, where the expected score on the right
    equals the actual score s_A on the left:

    s_A = w + 0.5 * d; n = w + d + l; 400 = scale_factor;
    ...
    400 * log10((n/s_A) - 1) = R_opp - R_A, and therefore:
    ...
    if p == 0:
        ...
        D = -EloConfig.SCALE * log10((1.0 / p) - 1.0)
        # FIDE binds D between -800 and 800, or +/- 2*scale
        # so let's do that too for non inf values.
        D = min(2 * EloConfig.SCALE, D)
        D = max(-2 * EloConfig.SCALE, D)
